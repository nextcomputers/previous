...
uint64_t CFileTable::GetIDByPath(const char *path) {
    FILE_HANDLE* handle;
...
FILE_HANDLE* CFileTable::GetHandleByPath(const char *path) {
...
bool CFileTable::GetPathByHandle(FILE_HANDLE* handle, std::string &path) {
	tree_node_<FILE_ITEM>* node;

	node = GetItemByID(handle->Get());
	if (node != NULL) {
...
tree_node_<FILE_ITEM>* CFileTable::AddItem(const char* path) {
	...
	strncpy(item.path, path, strlen(path) + 1);  //path
	item.nPathLen = strlen(item.path);  //path length
	item.handle   = new FILE_HANDLE(m_nTableSize);
...
tree_node_<FILE_ITEM>* CFileTable::GetItemByID(uint64_t nID) {
	FILE_TABLE *pTable;
...
int RenameDirectory(const char *pathFrom, const char *pathTo) {
	...
	const char* dotFile  = PATH_SEPS ".";
	const char* backFile = PATH_SEPS"..";
