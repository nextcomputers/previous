...
static bool WideToCharMap(const wchar_t* Src,char *Dest,size_t DestSize,bool &Success);
static void CharToWideMap(const char* Src, wchar_t *Dest,size_t DestSize,bool &Success);

// In Unix we map high ASCII characters which cannot be converted to Unicode
// to 0xE000 - 0xE0FF private use Unicode area.
static const uint32_t MapAreaStart=0xE000;
...
static const uint32_t MappedStringMark=0xFFFE;
...
// wcstombs is broken in Android NDK r9.
  ...
  if (!WideToCharMap(Src,Dest,DestSize,RetCode))
  {
    ...
    const wchar_t *SrcParam=Src; // wcsrtombs can change the pointer.
...
bool WideToCharMap(const wchar_t *Src,char *Dest,size_t DestSize,bool &Success)
{
  // String with inconvertible characters mapped to private use Unicode area
  // must have the mark code somewhere.
  if (wcschr(Src,(wchar_t)MappedStringMark)==NULL)
    return false;
  ...
  {
    if (uint32_t(Src[SrcPos])==MappedStringMark)
    {
      SrcPos++;
      continue;
    }
    ...
    if (uint32_t(Src[SrcPos])>=MapAreaStart+0x80 && uint32_t(Src[SrcPos])<MapAreaStart+0x100)
      Dest[DestPos++]=char(uint32_t(Src[SrcPos++])-MapAreaStart);
    else
    {
      mbstate_t ps;
      memset(&ps,0,sizeof(ps));
      ...
      int Length=mbrlen(Dest+DestPos,MB_CUR_MAX,&ps);
        DestPos+=std::max(Length,1);
    }
  }
    Dest[std::min(DestPos,DestSize-1)]=0;
  return true;
}
#endif
...
// We use it for extended ASCII names in Unix.
void CharToWideMap(const char *Src,wchar_t *Dest,size_t DestSize,bool &Success)
{
  ...
  while (DestPos<DestSize)
  {
    ...
    if (res==(size_t)-1 || res==(size_t)-2)
    {
      ...
      if (uint8_t(Src[SrcPos])>=0x80)
      {
        ...
        Dest[DestPos++]=uint8_t(Src[SrcPos++])+MapAreaStart;
      }
      else
        break;
    }
...
u_int8_t* WideToRaw(const wchar_t *Src,u_int8_t *Dest,size_t SrcSize)
{
  for (size_t I=0;I<SrcSize;I++,Src++)
  {
    Dest[I*2]=(u_int8_t)*Src;
    Dest[I*2+1]=(u_int8_t)(*Src>>8);
    if (*Src==0)
      break;
  }
  return Dest;
...
wchar_t* RawToWide(const u_int8_t *Src,wchar_t *Dest,size_t DestSize)
{
...
void WideToUtf(const wchar_t *Src,char *Dest,size_t DestSize)
{
...
size_t WideToUtfSize(const wchar_t *Src)
{
  size_t Size=0;
  for (;*Src!=0;Src++)
    if (*Src<0x80)
      ...
      if (*Src<0x800)
        ...
        if ((u_int32_t)*Src<0x10000) //(uint) to avoid Clang/win "always true" warning for 16-bit wchar_t.
        {
          ...
          if ((u_int32_t)*Src<0x200000) //(uint) to avoid Clang/win "always true" warning for 16-bit wchar_t.
            Size+=4;
  return Size+1; // Include terminating zero.
}


bool UtfToWide(const char *Src,wchar_t *Dest,size_t DestSize)
{
  bool Success=true;
  long dsize=(long)DestSize;
  dsize--;
...
bool IsTextUtf8(const u_int8_t *Src,size_t SrcSize)
{
  while (SrcSize-- > 0)
  {
    u_int8_t C=*(Src++);
    int HighOne=0; // Number of leftmost '1' bits.
    for (u_int8_t Mask=0x80;Mask!=0 && (C & Mask)!=0;Mask>>=1)
      HighOne++;
...
bool IsTextUtf8(const u_int8_t *Src)
{
    return IsTextUtf8(Src,strlen((const char *)Src));
}

int wcsicomp(const wchar_t *s1,const wchar_t *s2)
{
#ifdef _WIN_ALL
  ...
  while (true)
  {
    wchar_t u1 = towupper(*s1);
    wchar_t u2 = towupper(*s2);
    if (u1 != u2)
      return u1 < u2 ? -1 : 1;
    if (*s1==0)
      break;
...
int64_t atoilw(const wchar_t *s)
{
  bool sign=false;
  ...
  return sign && int64_t(n)>=0 ? -int64_t(n) : int64_t(n);
}

int atoiw(const wchar_t *s) {
    return (int)atoilw(s);
}
