...
std::string _first_dirname(std::string path) {
	auto wcs = multibyteToWide(path.c_str());
	if (wcs == NULL) {
		return path.substr(0, path.find(PATH_SEP));
	}
	auto wpath = std::basic_string<wchar_t>(wcs);
	delete wcs;
	auto dest = wideToMultiByte(wpath.substr(0, wpath.find(LPATH_SEP)).c_str());
	if (dest == NULL) {
		return path.substr(0, path.find(PATH_SEP));
	}
...
std::string _dirname_932(std::string path) {
	auto wcs = multibyteToWide(path.c_str());
	auto result = path.find(PATH_SEP) != std::string::npos ? path.substr(0, path.find_last_of(PATH_SEP)) : std::string("");
	if (wcs == NULL) {
		return result;
	}
	auto wpath = std::basic_string<wchar_t>(wcs);
	delete wcs;
	if (wpath.find(LPATH_SEP) == std::basic_string<wchar_t>::npos) {
		return result;
	}
	auto dest = wideToMultiByte(wpath.substr(0, wpath.find_last_of(LPATH_SEP)).c_str());
	if (dest == NULL) {
		return result;
	}
...
std::string _following_path(std::string path) {
	auto wcs = multibyteToWide(path.c_str());
	if (wcs == NULL) {
		return path.find(PATH_SEP) != std::string::npos ? path.substr(path.find(PATH_SEP) + 1) : std::string("");
	}
	auto wpath = std::basic_string<wchar_t>(wcs);
	delete wcs;
	if (wpath.find(LPATH_SEP) == std::basic_string<wchar_t>::npos) {
	...
	auto dest = wideToMultiByte(wpath.substr(wpath.find(LPATH_SEP) + 1).c_str());
	if (dest == NULL) {
		return path.find(PATH_SEP) != std::string::npos ? path.substr(path.find(PATH_SEP) + 1) : std::string(PATH_SEPS);
	}
	auto result = std::string(dest);
	delete dest;
	return result;
...
std::string _basename_932(std::string path) {
	...
	auto wpath = std::basic_string<wchar_t>(wcs);
	delete wcs;
	auto dest = wideToMultiByte(wpath.substr(wpath.find_last_of(LPATH_SEP) + 1).c_str());
	if (dest == NULL) {
		return path.substr(path.find_last_of(PATH_SEP) + 1);
	}
	auto result = std::string(dest);
...
FILE_ITEM CFileTree::AddItem(const char *absolutePath, FILE_HANDLE* handle) {
	FILE_ITEM item;
	item.handle  = handle;
	item.bCached = false;
...
void CFileTree::RenameItem(const char *absolutePathFrom, const char *absolutePathTo)
{
	...
	if (parentNode != NULL && node != NULL) {
		...
		std::string splittedPath = sPath.substr(sPath.find_last_of(PATH_SEP) + 1);
...
void CFileTree::GetNodeFullPath(tree_node_<FILE_ITEM>* node, std::string &path)
{
	...
	while (parentNode != NULL) {
		path.insert(0, PATH_SEPS);
		path.insert(0, parentNode->data.path);
		parentNode = parentNode->parent;
	}
}

void PrintFileTree(tree_node_<FILE_ITEM>* node, int level)
{
	if (CFileTree::debug) {
		printf("\n\n\n<<<<<<<<<<<<<<<<<<<<<DISPLAY tree \n\n\n");
		...
		while (sib2 != end2) {
			...
			if (tree<FILE_ITEM>::number_of_children(sib2) > 1) {
				PrintFileTree(sib2.node, (level + 1));
			}
			++sib2;
		}
		printf("\n\n\n<<<<<<<<<<<<<<<<<<<<<End tree \n\n\n");
