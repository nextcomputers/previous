...
void esp_start_command(Uint8 cmd) {
    ...
    if ((cmd&CMD_TYP_MASK)!=CMD_TYP_MSC) {
        if ((esp_state==TARGET && !(cmd&CMD_TYP_TGT)) ||
            ...
            esp_command_clear();
            intstatus |= INTR_ILL;
            CycInt_AddRelativeInterrupt(ESP_DELAY, INTERRUPT_ESP);
            return;
        }
    }
...
void esp_bus_reset(void) {
    ...
    if (!(configuration & CFG1_RESREPT)) {
        ...
        CycInt_AddRelativeInterrupt(wait*(32/ConfigureParams.System.nCpuFreq), INTERRUPT_ESP); /* CHECK: how is this delay defined? */
    } else {
...
void esp_select(bool atn) {
    ...
    if (timeout) {
        ...
        CycInt_AddRelativeInterrupt(seltout/**ConfigureParams.System.nCpuFreq*/, INTERRUPT_ESP);
        return;
    }
